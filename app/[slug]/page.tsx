...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://saasranker.net/${article.slug}` },
  }
}
...
export default function ArticlePage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    author: { '@type': 'Organization', name: 'SaaSRanker' },
    publisher: { '@type': 'Organization', name: 'SaaSRanker' },
  }
    ...
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <header className="mb-10">
          <span className="text-xs font-medium text-green-700 bg-green-50 px-2 py-0.5 rounded mb-3 inline-block">
            {article.category}
          </span>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400 border-t border-b border-gray-100 py-3">
            <span>By <strong className="text-gray-600">SaaSRanker Editorial</strong></span>
            <span>
              Updated:{' '}
              {new Date(article.updatedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
            </span>
            <span>{article.readingTime} min read</span>
            {article.verified && <span className="badge-verified">✓ Independently Verified</span>}
          </div>
        </header>
        ...
        {/* Comparison Table */}
        <section className="mb-10">
          <h2 className="section-heading">Tool Comparison</h2>
                  ...
                  <tr key={row.productName} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-4 py-3">
                      <a
                        ...
                        className="text-xs font-medium text-white bg-green-600 hover:bg-green-700 px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
                      >
                        Try it →
                      </a>
                    </td>
        ...
        {/* CTA */}
        <section className="rounded-2xl bg-green-950 text-white p-8 text-center">
          <p className="text-xs uppercase tracking-widest text-green-400 mb-2">Our Top Pick</p>
          <h3 className="text-xl font-serif font-bold mb-2">{article.ctaTitle}</h3>
          <p className="text-sm text-green-200 mb-6 max-w-md mx-auto leading-relaxed">{article.ctaDescription}</p>
          <a
            href={article.ctaAffiliateUrl}
            target="_blank"
            rel="nofollow noopener noreferrer"
            className="inline-block bg-white text-green-900 font-semibold px-6 py-3 rounded-xl hover:bg-green-50 transition-colors text-sm"
          >
